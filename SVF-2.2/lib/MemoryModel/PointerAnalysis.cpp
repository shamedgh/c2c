            ...
            ii != ie; ii++)
    {
        ...
        if(ObjPN* objPN = SVFUtil::dyn_cast<ObjPN>(pag->getPAGNode(*ii)))
        {
            ...
            if(obj->isFunction())
            {
                ...
                /// added for Temporal Type-based Pruning
                bool argMatch = applyTemporalTypeBasedPruning(calleefun, cs);
...
bool PointerAnalysis::applyTemporalTypeBasedPruning(const Function* calleefun, 
                                                    const CallBlockNode* cs){
    if ( !Options::TemporalTypeBasedPruning )
        return true;
    if ( calleefun->getFunctionType()->isVarArg() )
        return true;

    bool argMatch = true;
    unsigned argIndex = 0;
    for (llvm::Function::const_arg_iterator I = calleefun->arg_begin(), 
                                        E = calleefun->arg_end();
                                        I != E; ++I) {
        CallSite callSite = SVFUtil::getLLVMCallSite(cs->getCallSite());
        const llvm::Value *csArg = callSite.getArgument(argIndex);
        ...
        if (llvm::StructType* funcStTy = llvm::dyn_cast<llvm::StructType>(funcType)) {
            ...
            if ( !csStTy || !csStTy->isLayoutIdentical(funcStTy) ){
                if ( !csStTy ){
                    ...
                    std::string csStName = csStTy->getName().str();
                    std::string funcStName = funcStTy->getName().str();
